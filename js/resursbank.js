...
if (typeof ResursCheckout !== "undefined" && typeof omnivars !== "undefined" && omnivars !== null) {
    jQuery(document).ready(function ($) {
        if (omnivars["useStandardFieldsForShipping"] == "1") {
            console.log("Resurs Bank Note: useStandardFieldsForShipping (Experimental) is active, so customer fields are hidden rather than removed");
    ...
    resursCheckout.setCustomerChangedEventCallback(function(customerData) {
        if (omnivars["useStandardFieldsForShipping"] == "1") {
            console.log("Resurs Note: Received customer data update from iframe");
            ...
            if ($RB('#ship-to-different-address-checkbox').length > 0) {
                if (typeof delivery["firstname"] !== "undefined") {
                    ...
                    if (!$RB('#ship-to-different-address-checkbox').is(':checked')) {
                        console.log("Resurs Bank Note: Shipping checkbox is not checked and there is a delivery address set in the iframe. Shipping fields should enable.");
                        $RB('#ship-to-different-address-checkbox').attr("checked", false);
                        $RB('#ship-to-different-address-checkbox').click();
                    }
                } else {
                    if ($RB('#ship-to-different-address-checkbox').is(':checked')) {
                        console.log("Resurs Bank Note: Shipping checkbox is checked but there is no delivery address set in the iframe. Shipping fields should disable.");
                        $RB('#ship-to-different-address-checkbox').click();
                    }
                }
            }
            ...
            if (useShipping) {
                console.log("Shipping is now included!");
